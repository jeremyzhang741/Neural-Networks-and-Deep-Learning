...
def my_perceptron(x):
    ...
    """
    w = tf.get_variable("y", shape = x, initializer = tf.constant_initializer(1))
...
def onelayer(X, Y, layersize=10):
    ...
    """
    w = tf.Variable(tf.truncated_normal([784,layersize],stddev=0.1))
    b = tf.Variable(tf.zeros([layersize]))
    logits = tf.matmul(X, w) + b
    preds = tf.nn.softmax(logits)
    batch_xentropy = tf.nn.softmax_cross_entropy_with_logits_v2(labels = Y, logits = preds)
    batch_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels = Y, logits = preds))
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    """
    conv1 = tf.layers.conv2d(X, convlayer_sizes[0], filter_shape, 1, padding='same', activation=tf.nn.relu)
    conv2 = tf.layers.conv2d(conv1, convlayer_sizes[1], filter_shape, 1, padding='same', activation=tf.nn.relu)
    w = tf.Variable(tf.truncated_normal([28*28*10,outputsize], stddev=0.1))
    b = tf.Variable(tf.zeros([outputsize]))
    X_image = tf.reshape(conv2,[-1,28*28*10])
    logits = tf.matmul(X_image, w)+b
    preds = tf.nn.softmax(logits)
    batch_xentropy = tf.nn.softmax_cross_entropy_with_logits_v2(labels = Y, logits = preds)
    batch_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels = Y, logits = preds))
